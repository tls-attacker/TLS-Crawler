...
allResults = coll.find()#.limit(10000)
    ...
    if result['completedTimestamp'] != None and not 'failedWithException' in result['results']['tls_scan']:
        ...
        if result['results']['tls_scan']['attacks']['paddingOracleVulnerable']:
            ...
            for poresult in padddingOracleResults:
                suite = poresult['suite']
                eqError = poresult['getEqualityError']
                responseMapFp = ""
                for element in poresult['responseMap']:
                    responseMapFp += element
                responseMapFp = md5.new(responseMapFp).digest()

                if not (suite in buckets):
                    buckets[suite] = { }
                if not (eqError in buckets[suite]):
                    buckets[suite][eqError] = { }
                if not (responseMapFp in buckets[suite][eqError]):
                    buckets[suite][eqError][responseMapFp] = list()
                
                buckets[suite][eqError][responseMapFp].append(result['_id'] + ":" + result['targetIp'])
            ...
            if len(padddingOracleResults) > 1:
                ...
                if allVulEq:
                    allCiphersSameVuln.append(result['_id'] + ":" + result['targetIp'])
